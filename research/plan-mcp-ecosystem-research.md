...
# MCP & Agent-Skills Ecosystem — Research Plan

## Purpose

A research playbook to feed to Perplexity (Deep Research), OpenAI Deep Research, Claude Research, or a similar web-search-oriented AI agent, producing a deep-research document that grounds the AstroPrompt infrastructure thesis (candidate E in the decision framework) in the current state of the MCP / agent-skills / AI-tool-use market.

It answers the unresolved questions from `astroprompt-strategy-notes.md` §7–10 and the "distribution for the MCP classifier" open question from `astroprompt-decision-framework.md` v4.

## Why this research matters

The AstroPrompt strategy notes raise a larger question that the niche case sharpens: general-purpose LLMs often need specialized calculation or datasets to answer domain questions well (astrology, but also finance, sports, medical, legal, scientific). The classic path — build an app that wraps the calculation — assumes the user picks the specialized tool. The emerging path — MCP servers and agent skills — assumes the AI itself routes to the right specialized service.

If that emerging path is real and within reach in 2026, the AstroPrompt opportunity is less about building a sprawling app and more about building a **question-classifier + specialized-calculation layer** that any AI chat surface can call. Deciding which path to bet on requires understanding the current ecosystem, not just assuming it.

Expected output from the research brief below: a deep-research document that Daniel can read in one sitting and come out with (a) a current-state map of MCP + agent-skills + tool-use practice, (b) a clear picture of discovery and monetization patterns as they exist today, (c) comparable domain-specific plays, (d) the investment landscape, and (e) concrete strategic options for positioning a niche calculation provider.

## How to use this document

1. Open the research agent (Perplexity Deep Research, OpenAI Deep Research, Claude Research, or similar).
2. Copy everything under **"The research brief"** below as the agent's instructions.
3. Optionally add any Daniel-specific framing before submitting (e.g., "prioritize what a solo founder can realistically execute in 2026").
4. Expect a long output (10–30 pages with citations). Skim for structure first; drill into clusters of highest strategic interest.
5. Use **"Follow-up research"** at the bottom to run a second pass if the first is thin on any cluster.

---

## The research brief

> Copy everything from here to the end of this section into the research agent.
...
5. Credible dev-focused publications and newsletters (Latent Space, The Pragmatic Engineer, Simon Willison's blog, Hacker News discussion threads with named authors).
...
This research document feeds back into:

- **Decision framework v5 (or update):** the findings sharpen the matrix score for candidate E (Infrastructure), update the "what has to be true" preconditions for Pilot 3, and potentially reshape the strategic options table.
- **Pilot 3 design:** concrete choices about what to publish, where, and in what form (MCP-only, skill-only, both; open-source or gated; paid or free; marketed via paper, marketplace, blog, or silence).
- **The Katie Bohinc meeting:** context on whether her play is adjacent, competitive, or complementary to an infrastructure position.

Add a new note to `research/` with the deep-research document once produced, versioned and dated; update `astroprompt-decision-framework.md` in a subsequent revision based on the findings.
